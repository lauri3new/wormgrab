...
function getPoiDetails(poi, ACCS, res, cb) {
    try {
    ...
    request.get({
        ...
        if (error) {
            ...
            if ( body == false || body.length < 9) {return finished(new Error('no uniprot ids provided'))}
            else {
            baby.parse(body, {delimiter: "\t",newline: "\n", quotes:'false',
              complete: function(results) {
                 let geneName = '';
                 const WID = results.data[1][1];
                 // console.log('parserd', WID);
                 getDesc(WID);
                 function getDesc(WID) {
                ...
                function getPoiHomology(WID, family) {
                    ...
                    request.get({
                        ...
                        grab(WID, geneName, ACCS, family, orthologs, res, cb);
                        }
                    })
                }
        ...
        res.render('index', { error: '500 internal server error'});
    }
  }



function grab(WID, igene, ACCs, family = 'CaLpain', homologues = ['CAPN1',
'CAPN11',
'CAPN3',
'capn8',
'CAPN2',
...
var counter = 0;
  ...
  var searcharr = [['POI family name', 'POI homologous proteins'], [ family, ...homologues]];
    ...
    request.get({
        ...
        if (error) {
            ...
            if (body && body.esearchresult && body.esearchresult.idlist) {
              outarr[y].push(plus20(body.esearchresult.idlist.length));
